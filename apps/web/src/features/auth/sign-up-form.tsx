...
export default function SignUpForm({
  ...
  const handlePasswordSignup = async (event: React.FormEvent) => {
    ...
    if (signupError) {
      ...
      if (
        ...
        return toast.error(
          "This account uses OAuth or Magic Link. Sign in with that method, then add a password from Account settings."
        );
      }
  ...
  const hasSocialMethod =
    settings.githubSignUpEnabled ||
    settings.googleSignUpEnabled ||
  ...
  return (
    <div className="mx-auto mt-10 w-full max-w-md space-y-6 p-6">
      ...
      {error ? (
        ...
        >
          {errorDescription ||
            "Social sign-up could not be completed. If this email already has an account, sign in first and connect the provider from Account settings."}
        </div>
      ) : null}
      ...
      {settings.passwordSignUpEnabled && activeMethod === "password" ? (
        <section className="space-y-5">
          <div>
            <h2 className="text-lg font-semibold">Create with password</h2>
            <p className="text-sm text-muted-foreground">
              Create a verified account with a reusable password.
            </p>
          </div>
          <form onSubmit={handlePasswordSignup} className="space-y-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="password-signup-email">Email</Label>
              <Input
      ...
      {settings.magicLinkSignUpEnabled && activeMethod === "magic-link" ? (
        <section className="space-y-5">
          <div>
            <h2 className="text-lg font-semibold">Create with Magic Link</h2>
            <p className="text-sm text-muted-foreground">
          ...
          <form onSubmit={handleMagicSignup} className="space-y-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                placeholder="Your name"
                value={magicName}
                onChange={(event) => setMagicName(event.target.value)}
                autoComplete="name"
              />
            </div>
            <div className="space-y-2">
            ...
            >
              {isMagicPending ? "Sending..." : "Sign up with a magic link"}
            </Button>
          </form>
        </section>
      ) : null}
