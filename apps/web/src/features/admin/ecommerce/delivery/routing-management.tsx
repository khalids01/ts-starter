...
export function RoutingManagement({ connections, canManage, canDispatch, canManageReturns, canReconcile }: Props) {
  ...
  return (
    <Tabs defaultValue="services" className="space-y-3">
      ...
      <Dialog open={returnOpen} onOpenChange={setReturnOpen}><DialogContent><DialogHeader><DialogTitle>Record courier return request</DialogTitle></DialogHeader><p className="text-muted-foreground text-sm">Provider submission remains manual until the protected Steadfast return contract is verified.</p><Field label="Reason"><Input aria-label="Reason" value={operationForm.reason} onChange={(event) => setOperationForm({ ...operationForm, reason: event.target.value })} /></Field><DialogFooter><Button disabled={!operationForm.consignmentId || createReturn.isPending} onClick={() => createReturn.mutate()}>Record return</Button></DialogFooter></DialogContent></Dialog>
      <Dialog open={settlementOpen} onOpenChange={setSettlementOpen}><DialogContent><DialogHeader><DialogTitle>Record COD settlement evidence</DialogTitle></DialogHeader><div className="grid gap-4"><Field label="Payout/reference ID"><Input aria-label="Payout/reference ID" value={operationForm.externalId} onChange={(event) => setOperationForm({ ...operationForm, externalId: event.target.value })} /></Field><div className="grid grid-cols-2 gap-3"><Field label="Amount"><Input aria-label="Amount" value={operationForm.amount} onChange={(event) => setOperationForm({ ...operationForm, amount: event.target.value })} /></Field><Field label="Currency"><Input aria-label="Currency" value={operationForm.currency} onChange={(event) => setOperationForm({ ...operationForm, currency: event.target.value.toUpperCase() })} /></Field></div><Field label="Evidence note"><Input aria-label="Evidence note" value={operationForm.note} onChange={(event) => setOperationForm({ ...operationForm, note: event.target.value })} /></Field></div><DialogFooter><Button disabled={!operationForm.externalId.trim() || !operationForm.amount || operationForm.currency.length !== 3 || recordSettlement.isPending} onClick={() => recordSettlement.mutate()}>Record settlement</Button></DialogFooter></DialogContent></Dialog>
    </Tabs>
  );
}
